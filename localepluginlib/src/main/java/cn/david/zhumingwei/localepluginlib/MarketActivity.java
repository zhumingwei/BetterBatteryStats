...
public final class MarketActivity extends Activity
{
	...
	protected Dialog onCreateDialog(final int id)
	{
        ...
        if (null != compatiblePackage)
		{
			...
			for (ResolveInfo x : getPackageManager().queryIntentActivities(new Intent(cn.david.zhumingwei.localepluginlib.Intent.ACTION_EDIT_CONDITION), PackageManager.GET_ACTIVITIES))
			{
			...
			for (ResolveInfo x : getPackageManager().queryIntentActivities(new Intent(cn.david.zhumingwei.localepluginlib.Intent.ACTION_EDIT_SETTING), PackageManager.GET_ACTIVITIES))
			{
				ActivityInfo activityInfo = x.activityInfo;
				if (getPackageName().equals(activityInfo.packageName))
				{
